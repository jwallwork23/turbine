...
def construct_hessian(f, g=None, op=TurbineOptions()):
    """
...
def steady_metric(f, H=None, op=TurbineOptions()):
    """
    ...
    if op.normalisation == 'manual':
        ...
        for i in range(mesh.topology.num_vertices()):
            ...
            H_loc = H.dat.data[i] * op.target_vertices / max(np.sqrt(assemble(f * f * dx)),
                                                             f_min)  # Avoid round-off error
...
def normalise_indicator(f, op=TurbineOptions()):
    """
...
def isotropic_metric(f, bdy=None, invert=True, op=TurbineOptions()):
    """
...
def gradate_metric(M, iso=False, op=TurbineOptions()):  # TODO: Implement this in pyop2
    """
